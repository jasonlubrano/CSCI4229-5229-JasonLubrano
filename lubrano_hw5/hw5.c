	...
	glPushMatrix();
		...
		glBegin(GL_QUAD_STRIP);
			for (j = 0; j <= 360; j += DEF_D) {
				...
				glNormal3d((Cos(j))/sqrt(Cos(j)*Cos(j)+ Sin(j)*Sin(j)),
						+0,
						(Sin(j))/sqrt(Cos(j)*Cos(j)+ Sin(j)*Sin(j)));
		...
		for (i = len; i >= -len; i -= 1) {
			glBegin(GL_TRIANGLE_FAN);
				glColor3f(0.0, 0.0, 1.0);
				glNormal3d(0, len*i, 0);
				glVertex3f(0, len*i, 0);
				for (k = 0; k <= 360; k += DEF_D) {
					glColor3f(0.0, 0.0, 1.0);
					glNormal3d(Cos(k)/rad, i*len, Sin(k)/rad);
					glVertex3f(rad*Cos(k), len*i, rad*Sin(k));
				}
			glEnd();
		}
	glPopMatrix();
...
void display(){
	...
	switch(vpi){
		case 1:
			for(i = -10; i <= 10; i += 5)
			hydrolic_hw5(i, 0, 0, .25, .25, .25, spin*.5, spin*.5, spin*.5);
		break;
		case 2: hydrolic_hw5(0, 0, 0, 1, 1, 1, 0, 0, 0);
		break;
		case 3: plate_hw5(0, -5, 0, 1, 1, 1, 0, 0, 0);
		break;
		case 4: upperLeg_hw5(0, 0, 0, 1, 1, 1, 0, 0, 0);
		break;
		case 5:bottomLeg_hw5(0, 0, 0, 1, 1, 1, 0, 0, 0);
		break;
		default: break;
	}
...
void key(unsigned char ch,int x,int y) {
	...
	//  Reset view angle
		...
		vpi = 4; // upper cylinder
	else if (ch == '5')
		vpi = 5; //bottom cylinger
	//  Toggle axes
	else if (ch == 'x' || ch == 'X')
		axes = 1-axes;
	//  Toggle lighting
